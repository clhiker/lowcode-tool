...
if __name__ == "__main__":
    ...
    parser.add_argument("--miss_path", type=str, default="miss.txt", help="缺失文件列表 (default: miss.txt)")
    parser.add_argument("--result_file", type=str, default="fixed.md", help="输出目录，包含描述的md文件 (default: fixed.md)")
    ...
    with open(miss_path, 'r') as f:
        miss = f.read()
        miss_files = miss.split(', ')
        miss_files = [item.replace("'", '') for item in miss_files]
